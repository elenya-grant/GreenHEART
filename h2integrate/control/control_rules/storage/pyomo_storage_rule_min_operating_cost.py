...
class PyomoRuleStorageMinOperatingCosts:
    ...
    def __init__(
        ...
        # Define whether storage is allowed to buy commodity for charging
        self.allow_commodity_buying = allow_commodity_buying
    ...
    def initialize_parameters(self, inputs: dict, dispatch_inputs: dict):
        ...
        if self.allow_commodity_buying:
            commodity_buy_price_in = inputs[f"{self.commodity_name}_buy_price"]

        demand_met_value_in = inputs["demand_met_value"]
        ...
        self.set_timeseries_from_list("demand_met_value", demand_met_value_in)
    ...
    def _create_parameters(self, pyomo_model: pyo.ConcreteModel, t):
        ...
        # Add parameters that allow for buying commodity for storage charging
        if self.allow_commodity_buying:
            ...
            pyomo_model.commodity_import_limit = pyo.Param(
                doc=f"External commodity source import limit \
    ...
    def _create_variables(self, pyomo_model: pyo.ConcreteModel, t):
        ...
        if self.allow_commodity_buying:
            pyomo_model.commodity_bought = pyo.Var(
                ...
                domain=pyo.NonNegativeReals,
                bounds=(0, pyomo_model.commodity_import_limit),
                units=self.rate_units_pyo,
            )
    ...
    def _create_constraints(self, pyomo_model: pyo.ConcreteModel, t):
        ...
        pyomo_model.production_limit = pyo.Constraint(
            doc="Upper limit on system production",
        ...
        if self.allow_commodity_buying:
            pyomo_model.upper_purchase_limit = pyo.Constraint(
                ...
                expr=(
                    ...
                    <= pyomo_model.commodity_import_limit * (1 - pyomo_model.is_generating)
                ),
            )
    ...
    def update_time_series_parameters(
        ...
        self.set_timeseries_from_list(
            "commodity_load_demand", time_update_inputs[f"{self.commodity_name}_demand"]
        )
        # self.commodity_load_demand = [
        #   time_update_inputs[f"{self.commodity_name}_demand"][t] for t in self.blocks.index_set()
        # ]
        self.demand_met_value = [
            time_update_inputs["demand_met_value"][t] for t in self.blocks.index_set()
        ]
        ...
        if self.allow_commodity_buying:
            self.commodity_buy_price = [
                time_update_inputs[f"{self.commodity_name}_buy_price"][t]
    ...
    def min_operating_cost_objective(self, hybrid_blocks, tech_name: str):
        ...
        self.obj = sum(
            ...
            * (
                ...
                * self.blocks[t].demand_met_value
            )
            for t in self.blocks.index_set()
        )
        if self.allow_commodity_buying:
