...
def test_min_operating_cost_load_following_battery_dispatch(subtests):
    ...
    prob.setup()
    prob.prob.set_val("battery.electricity_in", electricity_in)
    prob.prob.set_val("battery.electricity_demand", demand_in)

    # Run the model
    prob.run_model()

    prob.get_val("battery.max_charge_rate", units="kW")[0]
    prob.get_val("battery.max_charge_rate", units="kW")[0]
    prob.get_val("battery.storage_capacity", units="kWh")[0]
    ...
    # # Check a longer portion of SOC to make sure SOC is getting linked between optimization
    # periods
